...
var request = function(protocol, options) {
	...
	this.boundary = options.boundary || "WS"
	this.length   = TWO_DASH.length * 2 + this.boundary.length;
	...
	if(options.url){
		...
		this.options.hostname = u.hostname
		this.options.path = u.pathname
	}
	...
	if (!headers[CONTENT_TYPE]) {
		headers[CONTENT_TYPE] = "multipart/form-data; boundary=" + this.boundary
	}
